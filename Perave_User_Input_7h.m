...
param.K = 3.5/sqrt(2); %e0*Bfield*/me/c/ku;                 % RMS undulator parameter
param.ku = 2.*pi./param.lambdau;                            % undulator wavenumber
lwig=param.lambdau*30;                                     % Undulator length m    
% Tapering options
param.tapering = 4;                                         % tapering (0 no tapering ; 1 KMR; 2 linear; 3 constant area; 4 polynomial)    
param.z0 = param.lambdau;
param.psir = 3*pi/8;
param.psirgradient = 27*1/lwig*pi/180;                      % 0 by default, Only used if param.tapering = 2 (linear psir taper)
param.dtaper = 1;                                           % taper exponent for Polynomial taper
deltaKoverK = 0.03;                                          % taper amplitude for polynomial taper
param.ctaper = deltaKoverK/((lwig-param.z0).^param.dtaper); % taper amplitdue (scaled)
...
P0 = 2.3e3; param.P0=P0;                                      % Seed power (W) 
...
energyspread = 33;                                            % Absolute RMS energy spread MeV
...
param.sigmat = 33*1e-18;                                      % beam sigma [s], only for gaussian (param.currprofile =1)
